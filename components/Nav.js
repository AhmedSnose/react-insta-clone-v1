...
function Nav() {
  ...
  return (
    <header
      className="sticky top-0 z-50 grid grid-cols-2 bg-white shadow-md py-5 px-3
        md:px-10"
    >
      {/* left */}
      ...
      <ul className="dropDownContiner flex items-center justify-self-end space-x-3">
        <HomeIcon
          className={`${
            router.pathname === "/profile" ? "text-green-100" : ""
          }`}
        />
        ...
        <NavItem
          icon={
            <Avatar
            //   src={url}
              className="hover:cursor-pointer max-h-[25px] max-w-[25px]"
            />
          }
        >
          <DopDown>
            <DropdownItem
              onClick={(e) => {
                e.preventDefault(), router.push("/profile");
              }}
            ...
            <DropdownItem
            onClick={(e) => {
              e.preventDefault(), router.push("/setting/changePic");
            }}
