...
function Post({ id, postUrl, caption, avaUrl, userName, place, timeStamp }) {
  ...
  return (
    <div className="POST_CONT flex flex-col my-2 shadow-lg bg-white ">
      {/* header */}
      ...
      <div
        ...
        className="flex items-center justify-center w-full mt-3 rounded-full"
      >
      ...
      <div className="flex flex-col w-full">
        ...
        <ul className="flex">
          {hasLiked ? (
            <Favorite
              onClick={addlike}
              className=" cursor-pointer text-red-700 mx-3 mt-3"
            />
          ) : (
          ...
          <SendOutlinedIcon onClick={Chat} className="cursor-pointer hover:text-blue-500 mx-3 mt-3" />
          <TurnedInNotOutlinedIcon className="mx-3 mt-3 ml-auto" />
        </ul>
        {likes.length > 0 && (
          <h3 className="font-bold text-[13px] mx-3 my-3">
            {likes.length} Likes
          </h3>
        )}
        {/* Caption */}
        ...
        <form className="flex items-center w-full border-t-[1px] border-opacity-5 border-black">
          ...
          <button
            ...
            onClick={addComment}
            className="text-blue-600 disabled:opacity-[.5] mr-3"
          >
            Post
          </button>
          {/* type='submit' */}
